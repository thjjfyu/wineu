        ...
        struct wine_device *device)
{
    ...
    *dst = *src;
    ...
    if (phys_dev->external_memory_align || phys_dev->map_placed_align || append_xr || have_ext_mem32 || have_ext_sem32 || have_keyed_mutex || append_timeline)
    {
        const char **new_extensions;
        unsigned int o = 0, count;

        count = dst->enabledExtensionCount;
        if (phys_dev->map_placed_align || phys_dev->external_memory_align)
            count += 2;
        if (append_xr)
            count += append_xr - 1;
        if (append_timeline)
        ...
        if (phys_dev->map_placed_align)
        {
            ...
            new_extensions[o++] = "VK_EXT_map_memory_placed";
            new_extensions[o++] = "VK_KHR_map_memory2";
        }
